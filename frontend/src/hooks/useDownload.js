...
export const useDownload = () => {
  ...
  const getDownloadInfo = useCallback(async (url) => {
    try {
      const response = await axios.get('/api/download/info', {
        params: { url }
      })
  ...
  const getFormats = useCallback(async (url) => {
    try {
      const response = await axios.get('/api/download/formats', {
        params: { url }
      })
      return response.data
    } catch (error) {
  ...
  // Batch download
  const batchDownload = useCallback(async (items) => {
    const ids = []
  ...
  useEffect(() => {
    ...
    const downloaded = downloads.reduce((sum, d) => sum + (d.downloadedSize || 0), 0)
